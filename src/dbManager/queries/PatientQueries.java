...
public class PatientQueries {
	...
	public static void insertPatient(Patient patient)	{
		...
		try {
			...
			PreparedStatement getID = con.prepareStatement("SELECT MAX(patientID) + 1 FROM Patient");
	...
	public static void deletePatient(int ID)	{
		...
		try {
			...
			pstmt.setInt(1, ID);
			pstmt.executeUpdate();
			pstmt = con.prepareStatement("UPDATE Patient SET patientID = patientID - 1 WHERE patientID > ?");
			pstmt.setInt(1, ID);
			pstmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public static void updatePatient(Patient patient)	{
		...
		PreparedStatement pstmt = null;
		try {
			pstmt = con.prepareStatement("UPDATE Patient SET title = ?, forename = ?, surname = ?, dateOfBirth = ?, phone = ?, houseNumber = ?, postCode = ? WHERE patientID = ?");
	...
	public static void main(String[] args)	{
		...
		Patient newPatient = new Patient(0, "Mr", "Curly", "Boi", DateHandler.newDate(1969, 07, 06), "0783649208", "-", "-");
		PatientQueries.insertPatient(newPatient);
		
		patients = PatientQueries.getAllPatients();
		System.out.println(patients);
		
		Patient updatedPatient = new Patient(1, "Mrs", "Curly", "Lass", DateHandler.newDate(1969, 07, 06), "0783649208", "-", "-");
